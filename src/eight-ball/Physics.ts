...
export class Physics extends Middleware<BilliardContext> {
  ...
  handleCueShot(data: { ball: Ball; shot: Vec2Value; spin?: { x: number; y: number } }) {
    ...
    if (data.spin) {
      const angularVelocity = data.spin.x * 8;
  ...
  handleFrameLoop(ev: { dt: number }) {
    ...
    if (!this.asleep) {
      ...
      for (let b = this.world.getBodyList(); b && asleep; b = b.getNext()) {
        if (!b.isStatic() && b.isAwake()) asleep = false;
      }
      this.asleep = asleep;
      if (this.asleep && this.context.shotInProgress) this.endShot();
    }
  }
  ...
  collide = (contact: Contact) => {
    ...
    if (pocket) contact.setEnabled(false);
    if (ball && pocket) {
      this.pocketedBalls.push(ball);
      const index = this.context.balls.indexOf(ball);
      if (index >= 0) this.context.balls.splice(index, 1);
    }
  };

  dataset = Dataset.create<Entity>({ key: (data) => data.key });
  ...
  ballDriver = Driver.create<Ball, Body>({
    ...
    exit: (data, body) => { this.world.destroyBody(body); },
  });

  railDriver = Driver.create<Rail, Body>({
    filter: (data) => data.type === "rail",
    enter: (data) => {
      const body = this.world.createBody({ type: "static", userData: data });
      body.createFixture({
  ...
  pocketDriver = Driver.create<Pocket, Body>({
    ...
    enter: (data) => {
      const body = this.world.createBody({ type: "static", position: data.position, userData: data });
      body.createFixture({
        shape: new Circle(data.radius),
        userData: data,
      });
